...
if (isset ( $_SESSION ["username"])&&isset ( $_SESSION ["userid"])){
    ...
    $id = $_GET['blogid'];
    $sql_blog = "select Usr.UsrId,UsrName,Class.ClassId,ClassName,Blog.BlogId,BlogTitle,BlogSum,BlogCont,BlogDate,BlogView from Usr, Blog, Class where Usr.UsrId = Blog.UsrId and Blog.ClassId = Class.ClassId and Blog.BlogId = '$id'";
    $result = $link ->query($sql_blog);
    list ($usrid,$blogusrname,$classid,$classname,$blogid,$blogtitle,$blogsum,$blogcont,$blogdate,$blogview) = $result->fetch_row();
...
<article>
    ...
    <div class="infosbox">
                <?php
                echo "<form method='post' action='change_blog1.php?blogid=$blogid' onsubmit='return checkForm()'>";
                echo "<div class='newsview'><h3 class='news_title'><strong>Title:</strong><br><input id='title' name = title value='$blogtitle' style='width: 200px;height: 25px' ></h3>";
                echo "<div class='news_about'><strong>Summary:</strong><br><textarea id='summary' name='summary'  style='height: 85px; width: 599px; margin: 0px;'>$blogsum</textarea></div>";
                echo "<div class='news_con'><strong>Content:</strong><br><textarea id='content' name='content'  style='height: 364px; width: 616px; margin: 0px;'>$blogcont</textarea></div>";
                ?>
